import Head from "next/head";
import Link from "next/link";

export default function Home() {
  return (
    <div>
      <Head>
        <title>Create Next App</title>
        <meta name="description" content="Generated by create next app" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <div className="pt-[140px] bg-gradient-to-l from-sparkGreen to-lime-400 w-full min-h-screen">
        <h1 className="text-white text-center mb-8">
          <svg
            className="inline h-[120px] fill-white drop-shadow-lg"
            xmlns="http://www.w3.org/2000/svg"
            xmlnsXlink="http://www.w3.org/1999/xlink"
            x="0px"
            y="0px"
            viewBox="0 0 415 415"
            xmlSpace="preserve"
          >
            <g>
              <g>
                <path d="M325.662,3.768C324.325,1.437,321.844,0,319.157,0H164.555c-3.218,0-6.078,2.053-7.107,5.102L91.19,201.421    c-0.772,2.289-0.394,4.81,1.014,6.772c1.409,1.962,3.677,3.126,6.093,3.126h62.812L88.817,404.876    c-1.278,3.422,0.096,7.268,3.254,9.106c1.18,0.686,2.48,1.018,3.769,1.018c2.16,0,4.287-0.932,5.756-2.687l201.228-240.49    c1.869-2.234,2.276-5.348,1.043-7.987c-1.232-2.639-3.882-4.326-6.795-4.326h-58.094l86.654-148.224    C326.988,8.966,327,6.099,325.662,3.768z M219.429,163.223c-1.356,2.32-1.368,5.187-0.03,7.518    c1.337,2.331,3.818,3.768,6.505,3.768h55.111L118.189,369.107l60.754-162.664c0.86-2.302,0.537-4.88-0.865-6.9    c-1.401-2.019-3.703-3.224-6.161-3.224h-63.173L169.939,15h136.145L219.429,163.223z" />
              </g>
            </g>
          </svg>
          <br />
          <span className="text-[80px] font-semibold leading-[80px] drop-shadow-lg">
            Spark
          </span>
          <br />
          <span className="text-[50px] font-semibold leading-[54px] drop-shadow-lg">
            Protocol
          </span>
        </h1>
        <p className="text-white text-[26px] px-8 drop-shadow-lg leading-[36px] mb-8">
          Spark protocol is the first protocol that allows electricity consumers
          to choose which source of energy they prefer.
        </p>
        <p className="text-center">
          <Link href="/my-profile">
            <span
              className={`inline cursor-pointer text-lg rounded-full border-2 text-sparkGreen border-sparkGreen bg-white px-2 py-1`}
            >
              {`Let's start →`}
            </span>
          </Link>
        </p>
      </div>
    </div>
  );
}
